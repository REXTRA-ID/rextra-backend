...
type SubFeature struct {
	...
	// BelongsTo Feature — sub fitur selalu milik satu fitur induk.
	Feature Feature `json:"feature,omitempty" gorm:"foreignKey:FeatureID;references:ID"`

	// HasMany Entitlement — dibutuhkan untuk menghitung berapa entitlement yang sudah
	// terbentuk dari sub fitur ini.
	Entitlements []Entitlement `json:"entitlements,omitempty" gorm:"foreignKey:SubFeatureID;references:ID"`
}

func (SubFeature) TableName() string {
	return "sub_features"
}
