...
func initCache() CacheService {
	...
	if redisHost != "" {
		...
		if err != nil {
			log.Printf("Failed to connect to Redis at %s:%s, falling back to memory cache: %v", redisHost, redisPort, err)
			return NewMemory()
		}
	...
	log.Println("REDIS_HOST not configured, using memory cache")
	return NewMemory()
}
