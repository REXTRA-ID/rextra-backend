...
// GetAllWithCounts mengambil semua Feature sekaligus dengan entitlement count per feature
// dan entitlement count per sub feature, dalam tiga query flat — menghindari N+1 saat GetAll di service.
// Return: (features dengan SubFeatures ter-preload, map entitlement count per featureID, map entitlement count per subFeatureID)
func (r *featureRepository) GetAllWithCounts(ctx context.Context, tx *gorm.DB) ([]entity.Feature, map[uuid.UUID]int64, map[uuid.UUID]int64, error) {
	...
	// Query 2: aggregate entitlement count per feature
	...
	// Query 3: aggregate entitlement count per sub feature
	// WHERE sub_feature_id IS NOT NULL memastikan hanya entitlement level sub_fitur yang dihitung
