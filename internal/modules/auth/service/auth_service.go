...
type (
	...
	authService struct {
		...
		membershipRepository     membership.UserMembershipRepository
...
func (s *authService) ForgetPassword(ctx context.Context, req dto_request.ForgetPasswordRequest) error {
	...
	if err != nil { return err }
	if !user.IsVerified { return errors.New("user not verified") }
	...
	if err := s.mailService.MakeMail("./internal/pkg/email/template/forget_password_email.html", map[string]any{
		...
		"Link":     token,
	}).Send(user.Email, "Forget Password").Error; err != nil { return err }
	return nil
}
...
func (s *authService) Login(ctx context.Context, req dto_request.LoginRequest) (dto_response.LoginResponse, error) {
	log.Println("[DEBUG] Start Login Process")
	
	user, err := s.userRepository.GetByEmail(ctx, nil, req.Email)
	if err != nil {
		log.Printf("[DEBUG] User not found: %v\n", err)
		if errors.Is(err, gorm.ErrRecordNotFound) { return dto_response.LoginResponse{}, myerror.InvalidCreds() }
		return dto_response.LoginResponse{}, err
	}

	log.Printf("[DEBUG] User found: %s\n", user.Email)

	if !user.IsVerified { 
		log.Println("[DEBUG] User not verified")
	...
	log.Println("[DEBUG] Password Match, generating token...")
	...
	if err != nil { 
		log.Printf("[DEBUG] GenerateToken failed: %v\n", err)
		return dto_response.LoginResponse{}, err 
	}

	log.Println("[DEBUG] Access Token generated")

	token, err := utils.RandomData(32)
	if err != nil { return dto_response.LoginResponse{}, myerror.ProcessingError(err) }

	if s.sessionRepository == nil {
		log.Println("[DEBUG] sessionRepository is NIL!")
		return dto_response.LoginResponse{}, errors.New("internal server error: session repository nil")
	}

	refreshToken, err := s.sessionRepository.Create(ctx, nil, entity.SessionToken{
		UserID: user.ID.String(), Token: token, ExpiresAt: time.Now().Add(30 * 24 * time.Hour),
		IsActive: true, AuthProvider: "local", DeviceInfo: nil,
	})
	if err != nil { 
		log.Printf("[DEBUG] Create session failed: %v\n", err)
		return dto_response.LoginResponse{}, myerror.DatabaseError(err) 
	}

	log.Println("[DEBUG] Login Successful")
	return dto_response.LoginResponse{AccessToken: accessToken, RefreshToken: refreshToken.Token, Role: string(user.Role)}, nil
}
...
func (s *authService) GetMe(ctx context.Context, userId string) (dto_response.GetMe, error) {
	...
	if err != nil { return dto_response.GetMe{}, err }
	return dto_response.GetMe{PersonalInfo: dto_response.PersonalInfo{ID: userId, Fullname: user.Fullname, Email: user.Email, PhoneNumber: user.PhoneNumber, Role: string(user.Role)}}, nil
}
...
func (s *authService) SendVerificationEmail(ctx context.Context, email string) error {
	...
	if err == gorm.ErrRecordNotFound || err != nil { return err }
	token, err := myjwt.GenerateToken(map[string]string{"user_id": user.ID.String(), "email": user.Email}, 24*time.Hour)
	if err != nil { return err }
	token = fmt.Sprintf("%s/auth/verify?token=%s", os.Getenv("APP_URL"), token)
	if err := s.mailService.MakeMail("./internal/pkg/email/template/verification_email.html", map[string]any{"Fullname": user.Fullname, "Verify": token}).Send(user.Email, "Verify Your Account").Error; err != nil { return err }
	return nil
}
...
func (s *authService) Logout(ctx context.Context, req dto_request.LogoutRequest) error {
	...
	if !session.IsActive { return myerror.New("session already inactive", myerror.Error_InvalidRequest) }
	session.IsActive = false
	_, err = s.sessionRepository.Update(ctx, nil, session)
	if err != nil { return myerror.DatabaseError(err) }
	return s.sessionRepository.Delete(ctx, nil, session)
}
