...
type (
	EntitlementRepository interface {
		...
		UpdateRestriction(ctx context.Context, tx *gorm.DB, model entity.Entitlement) (entity.Entitlement, error)
...
func (r *entitlementRepository) UpdateRestriction(ctx context.Context, tx *gorm.DB, model entity.Entitlement) (entity.Entitlement, error) {
	if tx == nil {
		tx = r.db
	}
	if err := tx.WithContext(ctx).
		Model(&model).
		Select("restriction_type", "token_cost", "updated_at").
