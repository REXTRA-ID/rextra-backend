...
func (r *userRepository) Create(ctx context.Context, tx *gorm.DB, user entity.User) (entity.User, error) {
	if tx == nil { tx = r.db }
	if err := tx.WithContext(ctx).Create(&user).Error; err != nil { return user, err }
	return user, nil
}
...
func (r *userRepository) GetById(ctx context.Context, tx *gorm.DB, userId string) (entity.User, error) {
	...
	if err := tx.WithContext(ctx).Take(&user, "id = ?", userId).Error; err != nil { return entity.User{}, err }
	return user, nil
}

func (r *userRepository) GetByIDs(ctx context.Context, tx *gorm.DB, ids []uuid.UUID) (map[uuid.UUID]entity.User, error) {
	if tx == nil { tx = r.db }
	var users []entity.User
	if err := tx.WithContext(ctx).Where("id IN ?", ids).Find(&users).Error; err != nil { return nil, err }
	result := make(map[uuid.UUID]entity.User, len(users))
	for _, u := range users { result[u.ID] = u }
	return result, nil
}
...
func (r *userRepository) GetByEmail(ctx context.Context, tx *gorm.DB, email string) (entity.User, error) {
	if tx == nil { tx = r.db }
	var user entity.User
	// Load Membership and Plan to avoid nil panic
...
func (r *userRepository) Update(ctx context.Context, tx *gorm.DB, user entity.User) (entity.User, error) {
	if tx == nil { tx = r.db }
	if err := tx.WithContext(ctx).Save(&user).Error; err != nil { return user, err }
	return user, nil
}
